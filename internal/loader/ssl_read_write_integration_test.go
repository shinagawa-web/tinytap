...
// buildSSLFreeHelper compiles testdata/ssl_free_helper.c with cc.
func buildSSLFreeHelper(t *testing.T, dir string) string {
	t.Helper()
	cc, err := exec.LookPath("cc")
	if err != nil {
		t.Skip("cc not available, skipping uprobe fixture test")
	}
	outPath := dir + "/ssl_free_helper"
	cmd := exec.Command(cc, "-o", outPath, "testdata/ssl_free_helper.c", "-ldl")
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("compile ssl_free_helper: %v\n%s", err, out)
	}
	return outPath
}

// TestAttachSSLReadWrite_RealFreeCall drives a real SSL_free(3) call in a
// subprocess against the system's real libssl.so.3 and confirms
// AttachSSLReadWrite's ringbuf carries an SSLOpFree event for the freed
// SSL* (#173) — the signal Pairer.CloseSSL needs to evict a pending
// fd-less request the moment its connection tears down, instead of only
// via Sweep's timeout.
func TestAttachSSLReadWrite_RealFreeCall(t *testing.T) {
	libsslPath := findLibSSL(t)
	...
	helperPath := buildSSLFreeHelper(t, t.TempDir())
	...
	if got.Op != events.SSLOpFree {
		t.Errorf("Op = %d, want SSLOpFree (%d)", got.Op, events.SSLOpFree)
	}
}
