...
- **Non-Ubuntu distros**: Fedora 43 (arm64, kernel 6.17.1) was tested in #213 — results in [`docs/compat-matrix.md`](compat-matrix.md). The full capability set holds unchanged: base three for plaintext, `+cap_sys_admin` for TLS, no `cap_syslog` on arm64. The `cap_sys_admin` requirement for TLS uprobe is confirmed non-Ubuntu-specific (#195 theory refuted). Debian 12, AlmaLinux 9, Alpine 3.23, Ubuntu 20.04, and WSL2 remain untested — see the matrix for status.
