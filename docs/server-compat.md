...
| nginx (static, `sendfile off`) | #44 | writev, header+body coalesced into 1+ iovecs | ✅ 200/200 | ⚠️ 512/8192 | ⚠️ 3328/51200 | ✅ placeholder confirmed live, 68/68 | Body lives in `iovec[1+]` — #111 is what makes it visible at all; visibility is far below the 4096 B cap (see notes) |
| nginx (reverse proxy) | #44 | writev, multiple calls per response, 1–4 iovecs each | ✅ 200/200 | ⚠️ 512/8192 | ⚠️ 14592/51200 | ✅ placeholder confirmed live, 68/68 | `proxy_pass` never touches sendfile; body arrives via nginx's ~4 KiB proxy buffer, re-emitted as many small `writev` calls |
| Caddy | #45 | — | — | — | — | — | |
...
- **nginx (#44)**: exercised on this Lima VM (arm64, nginx/1.28.0) across three configurations — static with `sendfile on` (the default), static with `sendfile off`, and reverse proxy (`proxy_pass` to a `python3 -m http.server` backend) — all with `tcp_nopush off`. Each was verified with `strace -f -tt -e trace=write,writev,sendto,sendmsg,sendfile,recvfrom,recvmsg` on the nginx worker plus the real `tinytap` TUI (driven non-interactively via `tmux send-keys`/`capture-pane`), for all four body sizes. No `ABANDONED` exchanges anywhere across any of the 12 request/response pairs; nginx never used chunked `Transfer-Encoding` in any of these tests since `Content-Length` is always known ahead of time for static files and buffered proxy responses, so #116/#122 don't apply here.
  ...
  - **`sendfile off`**: nginx coalesces headers and body into a single `writev` call with the body starting at `iovec[1]` (small/medium) or spanning two separate `writev` calls when the body is large enough to hit nginx's own internal buffer split (large.txt: 32768 B + 18432 B). This is exactly the scenario #111's multi-iovec fix targets, and it does make the body visible — but the *amount* visible is much less than the flat 4096 B cap because `bpf/tinytap.bpf.c`'s `iov_sample_budget()` gives each iovec index a small **fixed** budget (2816 / 512 / 256 / 128 / 128 / 128 / 64 / 64, summing to `MAX_PAYLOAD`), and sampling stops entirely for a syscall event the moment one iovec doesn't fully fit its own budget slot (a deliberate "clean prefix, no splicing" choice — see the comment on `fill_iov_payload`). Concretely: medium.txt's single 2-iovec `writev` (header 241 B + body 8192 B) samples the full header (fits `iovec[0]`'s 2816 B budget) plus only 512 B of body (`iovec[1]`'s budget) → **512/8192 visible**, not the ~3855 B a flat-cap reading would predict. large.txt gets two independent `writev` events, each restarting the budget schedule: the first (header + 32768 B body, 2 iovecs) contributes 512 B of body; the second (18432 B body, 1 iovec, no header) gets the full `iovec[0]` budget of 2816 B → **512 + 2816 = 3328/51200 visible**. small.txt (438 B total) and image.png (68 B body) fit entirely within `iovec[0]`+`iovec[1]`'s combined budget, so both are fully visible/placeholder-rendered.
  - **reverse proxy**: `sendfile` is never invoked regardless of the static-side `sendfile` setting — nginx's `proxy_pass` path reads the upstream response into its own proxy buffer (default ~4 KiB chunks) and re-`writev`s it downstream, issuing **many** `writev` calls per response once the body exceeds one buffer's worth (large.txt took 6 separate `writev` calls, 1–4 iovecs each). Each `writev` event independently restarts the same fixed budget schedule as the `sendfile off` case, so more (smaller) `writev` calls means more total sampled bytes: medium.txt (one 4-iovec `writev`: header + 3 body chunks) → 512/8192 visible, same as the `sendfile off` case, since sampling still stops after `iovec[1]`'s budget is exhausted regardless of how many iovecs follow. large.txt spreads across 6 `writev` calls (one header+body, five pure-body) → 512 + 2816×5 = **14592/51200 visible** — proportionally *more* visible than either static case, simply because proxy buffering happens to split the body into more separate syscalls, each getting a fresh `iovec[0]`-sized budget. tinytap also captures nginx's own outgoing request to the backend as a second, independent exchange (nginx acting as an HTTP/1.0 client) — worth noting as a two-hop capture artifact of proxying, not a bug.
